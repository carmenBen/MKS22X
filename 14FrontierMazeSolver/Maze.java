    ...
    */
	...
	for(int i = 0;i+1 < possibleMoves.length;i=i+2){
		...
		if(maze[x+possibleMoves[i]][y+possibleMoves[i+1]] == 'E' ){
			...
			end.setDistance(L.getOldDistance() + 1);
			end.setOldDistance(L.getOldDistance() + 1);
		    }
		    result[count] = end;
			...
			result[count].setDistance(Math.abs(end.getX() - result[count].getX()) + Math.abs( end.getY() - result[count].getY()) + L.getOldDistance() + 1);
			result[count].setOldDistance(L.getOldDistance() + 1);
		    }
		    setSpot(result[count],'?');
		    count++;
		}
